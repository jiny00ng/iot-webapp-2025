...
1. 뉴스, 게시글 수정
    ...
    - Create.cshtml과 Edit.cshtml에서 필요없는 `입력값 필드`(Writer, PostDate, ReadCNnt) 삭제
...
4. HTML 에디터 추가
    ...
    - 유사한 라이브러리 : Trumbowyg, CKEditor 5(기능 최대), summernote,TinyMCE(유료 존재), Quill(단순)
    - Trumbowyg 클라이언트 라이브러리 설치(NuGet패키지와 차이있음)
    - wwwroot > 마우스오른쪽 > 추가 > 클라이언트 쪽 라이브러리 추가 
    - Trumbowyg 검색 후 설치
    ...
    - Detail.cshtml, Delete.cshtml Westwind.AspNetCore.Markdown 라이블러리
...
5. EntityFramework로 자동 생성된 테이블 컬럼타입변경
    - LONGTEXT로 타입이 지정된 칼럼은 사용여부에 따라 VARCHAR(num)로 변경
...
6. 페이징
    ...
    - 저장프로시저는 CALL New_PagingBoard(1, 10), CALL New_PagingBoard(11, 20) 식으로 도출
...
- XML의 방대한 데이터크기 때문에 현재는 JSON으로 거의 이전(XML의 1/3 수준) 
- WebAPI 서버 사용처
...
9. Rrograms.cs AppDbContext 초기화 추가
...
- AJAX : Asyncronous Javascrit And Xml. 자브크립트에서 비동기로 메서드 호출하는 기술
    - 예전에는 XML로만 데이터 전달. 현재는 Json으로 이전 중...
...
- CORS Policy Block : Cross-Origin Resource Sharing. 다른 출처 리소스 접근허용 보안 메커니즘
    ...
    <img src="./image/web0033.png" width="450">
